...
const upcomingClubs: BookableItem[] = [
  {
    title: 'The Onyx Lounge',
    description: 'An opulent, multi-level virtual space defined by dark marble, neon accents, and deep house music.',
  ...
  },
  {
    ...
    description: 'A retro-futuristic dance floor under a shimmering constellation of lights. For classic disco and nu-funk beats.',
  ...
  },
  {
    ...
    description: 'An ethereal, nature-infused club where organic visuals pulse in time with ambient and melodic techno.',
    imageUrl: 'https://images.pexels.com/photos/932638/pexels-photo-932638.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
    price: 0.04,
  },
];

interface NightclubPageProps {
  isLoggedIn: boolean;
  onBook: (item: BookableItem, type: 'Nightclub' | 'Exhibition') => void;
}

const NightclubPage: React.FC<NightclubPageProps> = ({ isLoggedIn, onBook }) => {
  ...
  return (
    <div className="container mx-auto text-center animate-fade-in px-4 w-full">
      ...
      <p className="text-lg text-gray-700 dark:text-gray-300 max-w-2xl mx-auto mb-12">
        Secure your entry to our exclusive virtual venues. An unparalleled nightlife experience awaits.
      </p>
        ...
        {upcomingClubs.map((club) => (
          ...
          >
            <div className="relative">
              <img src={club.imageUrl} alt={club.title} className="w-full h-48 object-cover transition-transform duration-300 group-hover:scale-110" />
            </div>
            <div className="p-6 text-left flex flex-col flex-grow">
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{club.title}</h3>
              <p className="text-gray-600 dark:text-gray-400 flex-grow mb-6">{club.description}</p>
              <div className="flex justify-between items-center mt-auto">
                 <p className="text-xl font-bold text-cyan-500 dark:text-cyan-300">{club.price} ETH</p>
                 <div className="flex items-center space-x-2">
                    <button 
                        onClick={() => setImmersiveItem(club)}
                        disabled={!isLoggedIn}
                        ...
                        aria-label={`Enter VR/AR Experience for ${club.title}`}
                    >
                    ...
                    <button 
                        ...
                        disabled={!isLoggedIn}
                        className="font-semibold bg-fuchsia-500 text-white px-6 py-2.5 rounded-md hover:bg-fuchsia-400 transition-colors duration-300 transform hover:scale-105 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed disabled:transform-none"
                        aria-label={`Book now for ${club.title}`}
                    >
                        Book Now
                    </button>
                 </div>
